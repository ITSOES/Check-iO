...
Bags = defaultdict(Counter)
b = Counter()
qq = 1
def get_avg(counts):
    return sum(x*y for x, y in counts.items())/sum(counts.values())
        
def choose_good_gift(current_gift, gifts_in_bag, gift_number):
    
    # c_bag = Bags[gifts_in_bag]
    # c_bag.update([current_gift])
    b.update([current_gift])
    return get_avg(b) < current_gift * 2/gifts_in_bag
